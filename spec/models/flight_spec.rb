  ...
  RSpec.describe Flight, type: :model do
    ...
    before(:all) do
      ...
      flight.save
      Flight.create(from_id: 1, to_id: 2, dept_date: Time.at(Time.now.to_i + (24 * 60 * 60 * 1)))
      Flight.create(from_id: 4, to_id: 5, dept_date: Time.at(Time.now.to_i - (24 * 60 * 60 * 1)))
    end
    ...
    context ".search_flight" do
      context "search flight" do
        ...
        describe "with date" do
          ...
          it{ expect(Flight.search_flight(from_a, to_a, date).first).to eq(example)}
        end
      end
    end
